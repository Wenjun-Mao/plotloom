...
4. Write only JPEG or PNG outputs below the package’s `delivery/outputs/` directory. Do not edit source code, database files, canonical story data, package inputs, or any path outside the delivery directory.
...
6. Re-read the final manifest and compare its `jobId` and `requestHash` to `request.json`. The only success signal is a complete, hash-valid delivery receipt. Tell the coordinator the package and completion paths; do not claim Approval, reference selection, or a reviewed keyframe.
