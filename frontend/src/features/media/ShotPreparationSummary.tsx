...
export function ShotPreparationSummary({
  projectId, shot, storyboardRevision, draftChanged, review, workbench, mediaLoaded,
  onReview, onReturnToBridge,
}: {
  projectId?: string;
  shot: Shot;
  ...
  }, [projectId, storyboardRevision]);

  const current = sources?.projectId === projectId ? sources : undefined;
  ...
  const selectedKeyframe = mediaLoaded && approvalCurrent
  ...
  return <section className="notice shot-preparation-summary" data-testid="shot-preparation-summary" aria-label="当前镜头准备状态">
    ...
    <ul>
      ...
      <li>角色身份参考：{!mediaLoaded ? "正在读取" : referenceStatus.length ? referenceStatus.map((item) => `${item.characterId} ${item.status}`).join("；") : "此镜头无可见角色，不适用"}。{referenceStatus.length > 0 && <Button variant="quiet" onClick={() => ownerAction("shot-character-references")}>查看身份参考</Button>}</li>
      <li>美术参考：F3 场景/道具选择由美术参考工作区持有；当前镜头媒体准备未消费这些决定，不计为已选关键帧。</li>
      <li>审核关键帧：{!mediaLoaded ? "正在读取" : selectedKeyframe ? `当前选择 ${selectedKeyframe.assetId.slice(0, 8)}` : approvalCurrent ? "缺少当前审核关键帧" : "无当前批准，关键帧不可用"}。<Button variant="quiet" onClick={() => ownerAction("shot-keyframe-review")}>查看关键帧</Button></li>
