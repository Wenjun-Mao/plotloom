...
`minimax_h3_gateway.v1` adapter. Use the complete
...
2. `POST /v1/video-jobs` creates an asynchronous job using the sole frozen
   profile and mandatory `cover_center_crop`, `contain_pad`, or
...
fixed profile with an SSH-only edit: profile changes require a new versioned
