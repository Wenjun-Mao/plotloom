...
| --- | --- | --- |
| 1 — M1: one playable story | Direct Play entry; explicit start, complete opening/decision, both choices/endings, restart; loading/error handling; repeat after reopen without authoring UI | Implemented as a local candidate and awaiting director acceptance/push. The dedicated Play URL reuses canonical stages, selected-media jobs and the existing branching player; retained pilot and production fixture evidence are recorded in the M1 receipt. |
