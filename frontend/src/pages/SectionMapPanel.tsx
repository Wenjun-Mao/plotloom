    ...
    {!outline ? <p className="muted">先明确接受一个 outline，再建立章节与分支。</p> : <>
      ...
      <fieldset className="section-map-choice"><legend>唯一明确选择</legend>
        ...
        {mapping.choice.outcomes.map((outcome, index) => <div className="section-map-outcome" key={index}><strong>路径 {index + 1}</strong>
          ...
          <label>抵达结局<select disabled={readOnly || busy || Boolean(accepted)} value={outcome.endingSectionId} onChange={event => updateOutcome(index, "endingSectionId", event.target.value)}>{endingSections.map(section => <option key={section.sectionId} value={section.sectionId}>{section.title || section.sectionId}</option>)}</select></label>
        </div>)}
      </fieldset>
      ...
      {accepted && status === "current" && <Button variant="primary" disabled={readOnly || busy} onClick={onInstall}>{graphAdmission?.status === "current" ? `重新安装路由图 r${graphAdmission.graphRevision}` : "安装到规范路由图"}</Button>}
      {graphAdmission && <small>规范路由图 r{graphAdmission.graphRevision} · {graphAdmission.status === "current" ? "当前" : `过期：${graphAdmission.staleReasons.join("；")}`}</small>}
      {routes.length > 0 && <section className="section-map-routes" data-testid="section-map-route-cards"><strong>规范路由图</strong>{routes.map((route, index) => <article key={route.id}><span>路径 {index + 1}</span><strong>{route.label}</strong><small>{route.nodeIds.join(" → ")}</small></article>)}</section>}
    </>}
  </article>;
}
