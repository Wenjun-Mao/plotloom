...
| F3 — novel-art **(F3A text-first candidate/review path delivered; [receipt](../verification/2026-09-18-f3a-art-review-receipt.md); F3B reference proof pending)** | Shared locations/props, visual anchors and justified state/lighting variants | F3A freezes source/outline/map/graph/cast inputs, validates and renders one upstream-shaped text candidate, preserves stable scene/prop IDs and section usage through accept/reopen/save/restart, and makes cancellation reachable. No images, managed assets, or selection are claimed. F3B must prove inspectable reusable environment/prop references and surface missing/changed currentness. No physical interpretation inferred solely from an ambiguous state label. | Replace duplicated asset-authoring rules; retain managed asset storage/selection. |
