...
export type ProductionBridgeIntentMethod = "pending_inference.v1" | "model_inference.v1" | "author_reviewed.v1" | "source_excerpt_seed.v1";
export interface ProductionBridgeIntentEntry { id: string; targetKind: "scene_objective" | "beat_purpose"; targetId: string; sourceCoordinates: Record<string, unknown>; sourceContentHash: string; method: ProductionBridgeIntentMethod; suggestedText: string; text: string; }
export interface ProductionBridgeIntentPackage { method: ProductionBridgeIntentMethod; entries: ProductionBridgeIntentEntry[]; provenance?: Record<string, unknown> | null; }
...
export interface ProductionBridgeState { proposal: ProductionBridgeProposal | null; status: "missing" | "ready" | "accepted" | "stale"; staleReasons: string[]; installedStageRevisions: Record<string, number> | null; intentJob?: ProductionBridgeIntentJob | null; }
