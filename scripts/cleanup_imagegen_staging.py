...
def _task_staging_root() -> Path:
    ...
    raw_task_id = os.environ.get("CODEX_THREAD_ID") or ""
    if not raw_task_id:
        raise CleanupError("CODEX_THREAD_ID is required to prove task-owned staging")
    if raw_task_id != raw_task_id.strip():
        raise CleanupError("CODEX_THREAD_ID must be one path-safe task identifier")
    task_id = raw_task_id
    ...
    if (
        ...
        or any(separator and separator in task_id for separator in separators)
    ):
        raise CleanupError("CODEX_THREAD_ID must be one path-safe task identifier")
...
def _open_directory_fd(path: Path, *, label: str, require_private: bool = False) -> int:
    ...
    if require_private and (
        ...
        raise CleanupError(f"{label} must be private to the current user")
    return directory_fd


def _lock_directory_fd(directory_fd: int, *, label: str) -> None:
    """Refuse competing cleanup rather than racing mutable directory entries."""

    try:
        fcntl.flock(directory_fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except OSError as error:
        raise CleanupError(f"{label} is busy; preserve staging until its writer releases the lock") from error
...
def _read_regular_at(directory_fd: int, filename: str, *, label: str, max_bytes: int = 1_000_000) -> bytes:
    entry = _open_regular_at(directory_fd, filename, label=label)
    assert entry is not None
    fd, details = entry
    try:
        if details.st_size > max_bytes:
            raise CleanupError(f"{label} exceeds its maximum size")
        content = bytearray()
        os.lseek(fd, 0, os.SEEK_SET)
...
def cleanup_staging(*, package: Path, staging_root: Path, staged_paths: list[Path]) -> CleanupResult:
    ...
    outputs_root = _directory(delivery_root / "outputs", label="delivery outputs")
    candidates = [_confined_direct_child(path, staging_root, label="staged output") for path in staged_paths]

    package_fd = _open_directory_fd(package, label="package")
    staging_fd = _open_directory_fd(staging_root, label="staging root", require_private=True)
    delivery_fd = _open_directory_fd(delivery_root, label="delivery", require_private=True)
    outputs_fd = _open_directory_fd(outputs_root, label="delivery outputs", require_private=True)
    try:
        # The specialist creates private task directories before generation; all
        # cooperating producers release these locks before publishing a complete
        # manifest. Holding the ordered locks makes the final name unlink part of
        # the same exclusive transaction as descriptor-based verification.
        _lock_directory_fd(staging_fd, label="staging root")
        _lock_directory_fd(delivery_fd, label="delivery")
        _lock_directory_fd(outputs_fd, label="delivery outputs")
        try:
            request = json.loads(_read_regular_at(package_fd, "request.json", label="package request"))
            manifest = ImageDeliveryManifest.model_validate_json(
                _read_regular_at(delivery_fd, "completion.json", label="completion manifest")
            )
