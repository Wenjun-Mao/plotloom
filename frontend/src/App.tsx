...
function SettingsDialog({ settings, sessionKey, busy, onSettings, onSessionKey, onClose, onSave }: { settings: ProviderSettings; sessionKey: string; busy: boolean; onSettings: (settings: ProviderSettings) => void; onSessionKey: (key: string) => void; onClose: () => void; onSave: () => Promise<void> }) {
  ...
  return <div className="modal" role="dialog" aria-modal="true" aria-labelledby="settings-title"><button className="modal-backdrop" aria-label="关闭" onClick={onClose} /><section className="modal-card"><header><div><span>Provider boundary</span><h2 id="settings-title">供应商与会话 Key</h2></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="modal-body"><div className="notice"><strong>Key 不进入项目</strong><span>仅写入当前标签页的 sessionStorage，并作为 `X-Plotloom-Session-API-Key` 请求头发送。</span></div>{field("textProvider", "文本供应商")}{field("textBaseUrl", "文本 API 根地址")}{field("textModel", "文本模型")}{field("imageProvider", "图像供应商")}{field("imageBaseUrl", "图像 API 根地址")}{field("imageModel", "图像模型")}{field("videoProvider", "视频供应商")}{field("videoBaseUrl", "视频 API 根地址")}{field("videoModel", "视频模型")}<label><span>临时 API Key</span><input type="password" autoComplete="off" value={sessionKey} placeholder={serverKeys.length ? "留空使用服务器密钥" : "当前标签页的临时密钥"} onChange={(event) => onSessionKey(event.target.value)} /><small>{keyHint}</small></label></div><footer><Button variant="quiet" onClick={onClose}>取消</Button><Button variant="primary" disabled={busy} onClick={() => void onSave()}>{busy ? "正在保存…" : "保存设置"}</Button></footer></section></div>;
}
