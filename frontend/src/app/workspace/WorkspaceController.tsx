...
export default function WorkspaceController() {
  ...
  const page = useMemo(() => {
    switch (activePage) {
      case "brief": return <BriefPage key={`${editorRevisionKey(project, "brief")}:${recovery.editorNonce}`} value={recoveredValue("brief", project.brief)} bible={project.storyBible} graph={project.storyGraph} saving={authoring.projectSaving || projectReadOnly} proposalRunning={Boolean(running && run?.requestedStages.some((stage) => stage === "story_bible" || stage === "story_graph"))} proposalReady={Boolean(stageHeads.story_bible?.status === "ready" && stageHeads.story_graph?.status === "ready" && !project.staleStages.includes("story_bible") && !project.staleStages.includes("story_graph"))} onSave={(brief) => authoring.commitProject({ brief })} onGenerateProposal={async (brief) => { const projectId = await authoring.commitProject({ brief }); if (projectId) await commands.startProposal(projectId); }} onDraftChange={(value) => authoring.rememberDraft("brief", value)} onReviewStage={(stage) => workspaceNavigation.requestNavigation({ project: navigationProjectId, stage })} onContinueToPlanning={() => workspaceNavigation.requestNavigation({ project: navigationProjectId, stage: "beats" })} />;
