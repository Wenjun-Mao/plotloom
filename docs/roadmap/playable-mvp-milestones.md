...
| F6 — Audiovisual qualification **([readiness receipt](../verification/2026-09-18-f6-h3-audiovisual-readiness.md) and [offline 5/8-second contract receipt](../verification/2026-09-18-f6-h3-eight-second-offline-contract.md), with fresh clean-clone verification at `09b0d50`; not generated-output proof, human audiovisual review, or production acceptance)** | Test native H3 dialogue, speaker attribution, cross-clip voice consistency and prompt effectiveness | Plotloom now admits only explicit 5-second default and qualified 8-second H3 I2V contracts; the immutable job owns duration/frames/fps rather than a profile default. The deployed gateway's 5--15 parser remains broader than product capability. Next, use at most two isolated, unselected 8-second I2V clips: a native-dialogue test and a conditional matching-speaker/reference comparison, with actual visual/audio review and honest reviewer attribution. The F5 2--8 second cut / 15-second segment policy is review-only and not a one-clip dispatch rule. Decide native audio sufficiency versus separate TTS/post-production before substantial dialogue spend. | No speculative TTS/lip-sync stack; no pad/trim/stitch workaround, duplicate queue, or F5-to-media compiler. Keep F5/F7 production-reference and mapping choices explicit. |
