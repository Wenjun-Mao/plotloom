...
| F2 — novel-characters **(F2A source-bound cast review delivered; [live receipt](../verification/2026-09-18-f2a-cast-review-receipt.md); F2B consumer seam delivered, [pilot proof blocked](../verification/2026-09-18-f2b-cast-identity-proof.md))** | Shared character bible, motivation, appearance/voice direction, identity references; existing ImageGen handoff | F2A prepares, refreshes, inspects and explicitly accepts one upstream `cast.json` bound to accepted source/outline/F1B sections, preserving a thin explicit consumer-ID seam. F2B freezes that accepted cast revision/hash and appearance direction through existing identity-reference/image-job currentness, with no duplicate media authority. The retained F2A pilot has no canonical Bible/visible approved shot, so it cannot honestly run the required two-image proof without an out-of-scope authoring setup. Text direction is not proof of generated voice consistency. | F2B must exercise existing identity review with actual images in a prerequisite-complete disposable project; no duplicate reference/media authority. |
