...
def _complete_delivery(path: Path, job: dict, content: bytes, delivery_id: str) -> None:
        ...
        json.dumps(
            {
                ...
                "actualPrompt": "Offline fixture delivery; no live generation occurred.",
...
def test_project_owned_image_handoff_isolated_across_restart_and_stales_after_intent_replacement(tmp_path: Path) -> None:
    ...
    intent = client.post(
        ...
        json={"role": "shot_keyframe", "identityIntent": "Preserve the fixture identity.", "sourceRefs": ["offline fixture"]},
    ).json()
    ...
    original = client.post(
        ...
        json={"approvalId": approval["id"], "shotId": shot["id"], "storyboardRevision": storyboard["head"]["revision"], "presentationChange": "Keep the approved fixture legible."},
    )
    ...
    candidate_intent = client.post(
        ...
        json={"role": "shot_keyframe", "identityIntent": "Candidate fixture identity.", "sourceRefs": ["offline fixture candidate"]},
    ).json()
    ...
    refinement = client.post(
        f"/api/v2/projects/{first_id}/image-jobs",
        json={"approvalId": approval["id"], "shotId": shot["id"], "storyboardRevision": storyboard["head"]["revision"], "parentCandidateAssetId": candidate["id"], "presentationChange": "Refine the selected fixture candidate."},
    )
    ...
    client.post(
        f"/api/v2/projects/{first_id}/managed-assets/{candidate['id']}/visual-intents",
        json={"role": "shot_keyframe", "identityIntent": "Replacement fixture identity.", "sourceRefs": ["replacement fixture"]},
    ).raise_for_status()
