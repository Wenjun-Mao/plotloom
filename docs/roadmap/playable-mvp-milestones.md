...
| F2 — novel-characters **(F2A source-bound cast review delivered; [live receipt](../verification/2026-09-18-f2a-cast-review-receipt.md); F2B identity proof pending)** | Shared character bible, motivation, appearance/voice direction, identity references; existing ImageGen handoff | F2A prepares, refreshes, inspects and explicitly accepts one upstream `cast.json` bound to accepted source/outline/F1B sections, preserving a thin explicit consumer-ID seam. The linked isolated live proof records UI cancellation/replacement, reopen/save, reload and production restart; it is technical evidence, not human creative approval or provider telemetry. Two distinct shots/poses retain approved identity reference; supplied reference can be retained; edits identify affected downstream work. Text direction is not proof of generated voice consistency. | F2B must exercise existing identity review with actual images; no duplicate reference/media authority. |
