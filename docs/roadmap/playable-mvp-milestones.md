...
| F5 — novel-storyboard **(one content-preserving split of the failed 17.5s `E02-06` grouping yields a valid review-only candidate: all 71 cuts and 22 dialogue bindings are unchanged, pinned gates and 90s/180s caps pass; the historical F0 currentness record cannot be freshly reread because the current reader rejects its old request shape; [receipt](../verification/2026-09-18-f5-storyboard-production-seam-receipt.md); not production or F5 product acceptance)** | Script-derived shots, reference-image needs, timing, H3 production direction; reuse upstream reports/export | The derived candidate preserves the 22 script-owned dialogue lines, passes all upstream cut/segment gates, and meets each 90s section plus 180s complete-route cap. Candidate `maxCutSeconds` is configurable upstream; production H3 remains fixed5 until separately qualified. The current-reader historical-schema incompatibility is an evidence gap, not a waiver or candidate repair. No V2 SceneBeats projection is a prerequisite for review-only storyboard evidence. | Consolidate duplicate shot/prompt compilers; preserve review and candidate currentness. |
