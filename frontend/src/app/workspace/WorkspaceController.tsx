...
export default function WorkspaceController() {
  ...
  // A source fragment remains source-owned for loading and persistence, but its
  // validated destination is the creator's current task in the top bar.
  ...
  const page = useMemo(() => {
    switch (activePage) {
      ...
      case "brief": return <BriefPage key={`${editorRevisionKey(project, "brief")}:${recovery.editorNonce}`} value={recoveredValue("brief", project.brief)} bible={project.storyBible} graph={project.storyGraph} saving={authoring.projectSaving || projectReadOnly} proposalRunning={Boolean(running && run?.requestedStages.some((stage) => stage === "story_bible" || stage === "story_graph"))} proposalReady={Boolean(stageHeads.story_bible?.status === "ready" && stageHeads.story_graph?.status === "ready" && !project.staleStages.includes("story_bible") && !project.staleStages.includes("story_graph"))} storyboardRunning={Boolean(running && run?.requestedStages.some((stage) => stage === "scene_beats" || stage === "storyboard"))} onSave={(brief) => authoring.commitProject({ brief })} onGenerateProposal={async (brief) => { const projectId = await authoring.commitProject({ brief }); if (projectId) await commands.startProposal(projectId); }} onGenerateStoryboard={async () => { if (project.id) await commands.startStoryboard(project.id); }} onDraftChange={(value) => authoring.rememberDraft("brief", value)} onReviewStage={(stage) => workspaceNavigation.requestNavigation({ project: navigationProjectId, stage })} onContinueToPlanning={() => workspaceNavigation.requestNavigation({ project: navigationProjectId, stage: "beats" })} />;
