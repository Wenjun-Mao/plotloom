...
| 4 | Partial — production gate passed and one fresh Chinese 9:16 project reached an isolated text-generation blocker; no media work started | [Readiness preflight](../verification/2026-09-15-step-4-two-shot-pilot.md); [bounded pilot attempt](../verification/2026-09-15-step-4-two-shot-pilot-attempt.md) | A Scene Beats unit exhausted its bounded attempts with `semantic.invalid_continuity_entity_state`; the sanctioned exact repair then refused pre-dispatch because its parent evidence lacked one immutable candidate | Preserve the project and exact evidence; diagnose the authoring/repair contract before any new pilot or broad rerun |
