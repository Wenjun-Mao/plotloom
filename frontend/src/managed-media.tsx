...
export function ManagedMediaWorkbench({
  ...
  const proposalCandidates = useMemo(
    ...
    [characterProposals, proposalCharacterId],
  );
      ...
      >
        ...
        <div className="button-row">
          <Button
            ...
            onClick={() => void selectCharacterReference()}
          >
            选择身份参考
          </Button>
                ...
                {decision && (
                  <div className="button-row">
                    ...
                    <Button
                      ...
                      disabled={readOnly || busy}
                      onClick={() =>
                        void revokeCharacterReference(character.id)
                      }
                    >
                      撤销
                    </Button>
        ...
        <details className="image-job-history">
          ...
          <div className="button-row">
            <Button
              ...
              onClick={() => void prepareCharacterReferenceProposal()}
            >
          ...
          {characterProposals.map((proposal) => (
            <article
              key={proposal.id}
            ...
            >
              ...
              <div className="button-row">
                <Button
                  ...
                  onClick={() =>
                    void copyCharacterReferenceProposal(proposal.id)
                  }
                >
                ...
                <Button
                  ...
                  disabled={readOnly || busy}
                  onClick={() =>
                    void refreshCharacterReferenceProposal(proposal.id)
                  }
                >
                  检查 delivery
                </Button>
            ...
            >
              ...
              <div className="button-row">
                <Button
                  ...
                  onClick={() => void copyImageJob(job.id)}
                >
                  Copy assignment
                </Button>
                <Button
                  ...
                  variant="quiet"
                  disabled={readOnly || busy}
                  onClick={() => void refreshImageJob(job.id)}
                >
                  检查 delivery
                </Button>
                <Button
      ...
      <div className="field-grid two compact">
        ...
        <Field label="已知新增内容（每行一项）">
          <input
            ...
            disabled={readOnly || busy}
            onChange={(event) => setDeclaredAdditions(event.target.value)}
          />
        </Field>
      ...
      <div className="media-candidate-grid" aria-label="候选图像比较">
        {workbench.assets.map((asset) => (
          <CandidateCard
            ...
            onPick={() => chooseCandidate(asset.id)}
            onKeep={() => keepCandidate(asset.id)}
          />
        ))}
        {!workbench.assets.length && (
        ...
        >
          ...
          <div className="button-row">
            <Button
              ...
              onClick={() => void recordSamePersonReview()}
            >
              记录视觉复核
            </Button>
          </div>
    ...
    <article className={`media-candidate ${selected ? "selected" : ""}`}>
      <button onClick={onPick} aria-pressed={selected}>
        ...
        {asset.provenance?.declaredAdditions.length ? (
          <small>
            已知新增：{asset.provenance.declaredAdditions.join("、")}
          </small>
        ) : null}
      </button>
      <Button
