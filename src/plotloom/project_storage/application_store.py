...
class ApplicationStore:
    ...
    @staticmethod
    def _dispatch_from_row(row: sqlite3.Row) -> VideoDispatchLease:
    ...
    def initialize_video_accounting(self, *, limit_units: int) -> None:
        ...
        with self._write() as connection:
            ...
            if existing is not None:
                raise ProjectStorageConflictError("direct video accounting is already initialized")
            connection.execute(
    ...
    def reserve_video_dispatch(
        ...
        *,
        ...
        """Reserve one durable identity before its project can dispatch.

        A repeat is idempotent only when every immutable accounting attribute
        matches.  Paid work requires an explicitly initialized ledger; an
        absent ledger is a refusal, never an implicit 100-unit allowance.
        """
        ...
        with self._write() as connection:
            ...
            if existing is not None:
                lease = self._dispatch_from_row(existing)
                if (
                    lease.resource != resource
                    or lease.reserved_units != reserved_units
                    or (requires_accounting and reserved_units == 0)
                ):
                    raise ProjectStorageConflictError("video dispatch identity conflicts with its immutable reservation")
                return lease
            if requires_accounting:
                ...
                if ledger is None:
                    raise ProjectStorageError("paid video dispatch is disabled: application accounting is not initialized")
                if ledger["reserved_units"] + reserved_units > ledger["limit_units"]:
                    raise ProjectStorageConflictError("video requested-unit allowance would be exceeded")
                connection.execute(
    ...
    def record_video_dispatch_claim(self, dispatch_identity: str) -> VideoDispatchLease:
        """Record the persisted project claim before any provider operation."""
        ...
        with self._write() as connection:
            ...
            lease = self._dispatch_from_row(row)
    ...
    def release_video_before_dispatch(self, dispatch_identity: str) -> VideoDispatchLease | None:
        """Release only an identity that never received a dispatch claim."""
        ...
        with self._write() as connection:
            ...
            if lease.reserved_units:
                ...
                if updated.rowcount != 1:
                    raise ProjectStorageError("video accounting reservation cannot be safely released")
            connection.execute(
