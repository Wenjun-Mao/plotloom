  ...
  return <section className="reference-subject-gallery" aria-labelledby="reference-subject-title">
    ...
    <section className="appearance-workspace" aria-label={`${subject.name} 的外观工作区`}>
      ...
      <div className="appearance-thumbnails" aria-label="已有图片">
        {viewableCandidates.map((candidate) => <button key={candidate.id} type="button" className={`appearance-thumbnail${candidate.assetId === effectiveViewedAssetId ? " viewing" : ""}${selectedAssetIds.has(candidate.assetId) ? " selected" : ""}`} aria-pressed={candidate.assetId === effectiveViewedAssetId} onClick={() => { setViewedAssetId(candidate.assetId); setComparisonAssetId(""); }}><AssetPresentation projectId={projectId} subjectId={subject.id} asset={candidate.asset ?? undefined} assetId={candidate.assetId} alt={`${candidate.role === "refinement" ? "细化" : "候选"}缩略图`} unavailableLabel="候选图片不可用" /><span>{selectedAssetIds.has(candidate.assetId) ? "当前身份参考" : candidate.role === "refinement" ? "细化" : "候选"}</span></button>)}
        {!viewableCandidates.length && <p className="reference-empty-list">没有可浏览的候选图片。</p>}
      </div>
      {viewableCandidates.length > 1 && <div className="appearance-compare-controls"><span>比较</span>{viewableCandidates.filter((candidate) => candidate.assetId !== effectiveViewedAssetId).map((candidate, index) => <Button key={candidate.id} variant="quiet" disabled={busy} onClick={() => setComparisonAssetId(candidate.assetId)}>与图片 {index + 1} 对比</Button>)}</div>}
