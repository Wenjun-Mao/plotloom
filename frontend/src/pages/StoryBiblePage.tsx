...
export function StoryBiblePage({ value, stale, saving, onSave }: { value: StoryBible; stale: boolean; saving: boolean; onSave: (value: StoryBible) => Promise<void> }) {
  const [draft, setDraft] = useState(value);
  const updateCharacter = (id: string, patch: Partial<CharacterCard>) => setDraft((current) => ({ ...current, characters: current.characters.map((card) => card.id === id ? { ...card, ...patch } : card) }));
  const addCharacter = () => setDraft((current) => ({ ...current, characters: [...current.characters, { id: crypto.randomUUID(), name: "新角色", role: "", description: "", goal: "", traits: [], visualIdentity: "", continuityRules: [] }] }));
  return <div className="page">
    ...
    <Panel className="form-card">
      <div className="field-grid two">
        <Field label="Logline"><textarea rows={4} value={draft.logline} onChange={(event) => setDraft({ ...draft, logline: event.target.value })} /></Field>
        <Field label="故事前提"><textarea rows={4} value={draft.premise} onChange={(event) => setDraft({ ...draft, premise: event.target.value })} /></Field>
        <Field label="统一视觉语言"><textarea rows={4} value={draft.visualLanguage} onChange={(event) => setDraft({ ...draft, visualLanguage: event.target.value })} /></Field>
        <Field label="叙事承诺"><textarea rows={4} value={draft.narrativePromise} onChange={(event) => setDraft({ ...draft, narrativePromise: event.target.value })} /></Field>
        ...
        <Field label="世界规则（每行一条）"><textarea rows={5} value={draft.worldRules.join("\n")} onChange={(event) => setDraft({ ...draft, worldRules: event.target.value.split("\n").filter(Boolean) })} /></Field>
      </div>
    </Panel>
    ...
    <div className="card-grid">
      {draft.characters.map((character, index) => <Panel className="character-card" key={character.id}>
        ...
        <Field label="稳定外观"><textarea rows={3} value={character.visualIdentity} onChange={(event) => updateCharacter(character.id, { visualIdentity: event.target.value })} /></Field>
