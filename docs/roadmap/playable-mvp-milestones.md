...
| 2 — Synopsis to proposal | Minimal input produces understandable characters, setting, dramatic direction, branches/endings and production scope; optional defaults and actionable unsupported-input feedback | Implemented locally; director acceptance pending. Reuses Brief/Bible/Graph and existing generation; browser evidence covers the proposal review loop before downstream production. [Map](checkpoint-2-synopsis-proposal-map.md). |
