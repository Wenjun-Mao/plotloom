"""Independent F2A cast review rows; upstream cast bytes remain JSON payloads."""
...
class CastHeadRow(Base):
    __tablename__ = "v2_cast_heads"
...
class CastCandidateRow(Base):
    __tablename__ = "v2_cast_candidates"
    ...
    expected_cast_revision: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    cast: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    report_html: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    delivered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))

class CastRevisionRow(Base):
    __tablename__ = "v2_cast_revisions"
    ...
    binding: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    cast: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    consumer_mappings: Mapped[list[dict[str, str]]] = mapped_column(JSON, nullable=False)
