...
| F6 — Audiovisual qualification **([readiness receipt](../verification/2026-09-18-f6-h3-audiovisual-readiness.md) delivered; not an adapter change, generated-output proof, human audiovisual review, or production acceptance)** | Test native H3 dialogue, speaker attribution, cross-clip voice consistency and prompt effectiveness | Read-only deployed V4 health/schema is ready and exposes 5--15 seconds, but current Plotloom production adapter remains fixed5: an 8-second test needs a small explicit adapter/output-contract change first. Then use at most two isolated, unselected 8-second I2V clips: a native-dialogue test and a conditional matching-speaker/reference comparison, with actual visual/audio review and honest reviewer attribution. The F5 2--8 second cut / 15-second segment policy is review-only and not a one-clip dispatch rule. Decide native audio sufficiency versus separate TTS/post-production before substantial dialogue spend. | No speculative TTS/lip-sync stack; no pad/trim/stitch workaround, duplicate queue, or F5-to-media compiler. Keep F5/F7 production-reference and mapping choices explicit. |
