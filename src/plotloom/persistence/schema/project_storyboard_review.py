"""F5A review rows; they intentionally are not canonical storyboard rows."""
...
class StoryboardReviewHeadRow(Base):
    __tablename__ = "v2_storyboard_review_heads"
...
class StoryboardReviewCandidateRow(Base):
    __tablename__ = "v2_storyboard_review_candidates"
    ...
    expected_review_revision: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    storyboard: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    report_html: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    delivered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))


class StoryboardReviewRevisionRow(Base):
    __tablename__ = "v2_storyboard_review_revisions"
    ...
    binding: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
    storyboard: Mapped[dict[str, Any]] = mapped_column(JSON, nullable=False)
